...
document.addEventListener("DOMContentLoaded", function(event) {
  var arrow = document.querySelector('.go-down');
  ...
  for (var i=0; i < portfolioItems.length; i++){
    ...
    portfolioItem.addEventListener('click', function(event){
      ...
      if (!hasClass(this.querySelector('.teaser'), 'hidden')) {
        scrollTo(portfolioItem.offsetTop + portfolioItem.offsetHeight, 500, easing.linear);
      }
    });
  }
});
